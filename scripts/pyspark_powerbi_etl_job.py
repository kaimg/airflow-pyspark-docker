...
def extract(spark, source_table):
    print(f"[PowerBI ETL - Extract] Reading from {source_table} via Spark Utils")
...
def load(df, target_table, mode="overwrite"):
    print(
        f"[PowerBI ETL - Load] Writing to {target_table} ({mode} mode) via Spark Utils"
    )
    jdbc_url = DB_CONFIG["jdbc_url"]
    properties = {
        "user": DB_CONFIG["user"],
...
def run_contracts_pipeline(**kwargs):
    ...
    spark = create_spark_session(app_name="PowerBI ETL Job")
    try:
