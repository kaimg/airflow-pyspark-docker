...
def transform_benchmarking_material_sql(
    spark,
    extracted_dfs,
    pipeline_name="benchmarking_material_pipeline",
    jdbc_url=None,
...
def run_benchmarking_material_pipeline(**kwargs):
    ...
    sql_file_path = kwargs.get(
        "sql_file_path",
        "dags/sql/benchmarking/benchmark_report_material_benchmark_history.sql",
    )
    ...
    spark = create_spark_session(app_name="Benchmarking Report Material Job")
    try:
        extracted_dfs = extract_tables(spark, pipeline_name, jdbc_url_source, db_properties_source)
        transformed_df = transform_benchmarking_material_sql(
