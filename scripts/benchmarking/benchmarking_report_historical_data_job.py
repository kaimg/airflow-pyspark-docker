...
ETL_CONFIG = {
    "benchmarking_history_pipeline": {
        "source_tables": [
            {
                "name": "benchmark_report_historical_data",
                "schema": "public",
                "alias": "benchmark_report_historical_data",
            }
        ],
...
def transform_benchmarking_history_sql(
    spark,
    extracted_dfs,
    pipeline_name="benchmarking_history_pipeline",
    jdbc_url=None,
...
def run_benchmarking_history_pipeline(**kwargs):
    ...
    spark = create_spark_session(app_name="Benchmarking Report Historical Data Job")
    try:
        extracted_dfs = extract_tables(spark, pipeline_name, jdbc_url_source, db_properties_source)
        transformed_df = transform_benchmarking_history_sql(
