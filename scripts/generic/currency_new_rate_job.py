...
from scripts.spark_utils import create_spark_session, load_to_jdbc
...
def extract_tables(
    spark, pipeline_name="currency_rates_pipeline", jdbc_url=None, db_properties=None
):
    print(
...
def run_currency_rates_pipeline(**kwargs):
    ...
    try:
        extracted_dfs = extract_tables(
            spark, pipeline_name, jdbc_url=None, db_properties=None
        )
        transformed_df = transform_user_info_sql(
            spark,
            extracted_dfs,
            pipeline_name,
            jdbc_url=None,
